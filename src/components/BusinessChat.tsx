...
type Message = { role: 'user' | 'assistant'; text: string; researchId?: string; sources?: ResearchSource[]; researchStatus?: 'pending' | 'approved' | 'rejected' }
...
export function BusinessChat() {
  ...
  const ask = async (question: string) => {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: 'assistant', text: payload.answer, researchId: payload.research?.id, sources: payload.research?.sources, researchStatus: payload.research ? 'pending' : undefined }])
    } catch (error) {
  ...
  const reviewResearch = async (messageIndex: number, action: 'apply' | 'reject') => {
    ...
    setMessages((current) => current.map((entry, index) => index === messageIndex ? { ...entry, researchStatus: action === 'apply' ? 'approved' : 'rejected' } : entry))
  }
